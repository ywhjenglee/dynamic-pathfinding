...
public class MiddleBridge : MonoBehaviour
{
    ...
    void Update() {
        foreach (GameObject go in GameObject.FindGameObjectsWithTag("NPC")) {
            ...
            if (gs.getTransfer() == 2 && !inUse && (go.transform.position - new Vector3(0.5f, 1.25f, -1.5f)).magnitude <= 5f) {
                inUse = true;
